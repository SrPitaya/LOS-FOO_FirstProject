...
$query = "SELECT profesor.categoria AS Categorias, COUNT(profesor.idProfesor) AS Numero_Profesores
          FROM profesor
          GROUP BY profesor.categoria
          ORDER BY Numero_Profesores DESC
          LIMIT $limit OFFSET $offset";
$result = mysqli_query($conn, $query);

// Consulta para obtener el total de registros
$totalQuery = "SELECT COUNT(DISTINCT categoria) as total FROM profesor";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<td class='p-4'>" . $row['Categorias'] . "</td>";
    echo "<td class='p-4'>" . $row['Numero_Profesores'] . "</td>";
    echo "</tr>";
}
